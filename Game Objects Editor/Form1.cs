...
namespace Game_Objects_Editor
{
    public partial class Form1 : Form
    {
        ...
        List<string> weapons;
        List<string> weapons0;
        List<string> armors;
        List<string> armors0;
        List<string> abilities;
        List<string> abilities0;
        List<string> abilities1;
            ...
            foreach (string line in lines)
            {
                switch (reader)
                {
                    ...
                    case 12:
                        ...
                        break;
                    case 13:
                        agilityGainBox.Text = line;
                        break;
                    case 14:
                        actionsGainBox.Text = line;
                        break;
                    case 15:
                        maxEnergyGainBox.Text = line;
                        break;
                    case 16:
                        maxHealthGainBox.Text = line;
                        break;
                    case 17:
                        precisionGainBox.Text = line;
                        break;
                    case 18:
                        ability1Box.Text = line;
                        break;
                    case 19:
                        ability2Box.Text = line;
                        break;
                    case 20:
                        ability3Box.Text = line;
                        break;
                    default:
        ...
        void verifyCharacter()
        {
            ...
            if (isNumeric(agilityGainBox.Text) == false)
                warnings.Add(base0 + "Warning: Agility Gain must be Numeric");
            if (isNumeric(actionsGainBox.Text) == false)
                warnings.Add(base0 + "Warning: Actions Gain must be Numeric");
            if (isNumeric(maxEnergyGainBox.Text) == false)
                warnings.Add(base0 + "Warning: Max Energy Gain must be Numeric");
            if (isNumeric(maxHealthGainBox.Text) == false)
                warnings.Add(base0 + "Warning: Max Health Gain must be Numeric");
            if (isNumeric(precisionGainBox.Text) == false)
                warnings.Add(base0 + "Warning: Precision Gain must be Numeric");
        ...
        void exportFile()
        {
            ...
            //Basic Template:
            exportFile.WriteLine("#pragma once");
            exportFile.WriteLine("#include \"../../../Header Files/Objects/DataBase.h\"");
            ...
            exportFile.WriteLine("/****************************************************************************************************************************************/");
            exportFile.WriteLine("//                                                       CHARACTERS                                                                      //");
            exportFile.WriteLine("/****************************************************************************************************************************************/");


            //Populating Characters Database List
            exportFile.WriteLine("//Characters Stat Blocks: exp, maxHealth, Agility, Precision, MaxEnergy, Precision, Mastery, Actions Remaining");
            int characterID=0;
            foreach (string characterTXT in characterList)
            {
                string characterPath = "./Data/Characters/" + characterTXT;
                string[] lines = File.ReadAllLines(characterPath);
                string[] stats = {"exp","MaxHealth","Agility","Precision","MaxEnergy","Mastery","actionsremaining"};
                string[] statGains = { "AgilityGain", "MaxActionsGain", "MaxEnergyGain", "MaxHealthGain", "PrecisionGain" };
                exportFile.WriteLine("");
                exportFile.WriteLine("//Character ID: " + characterID+" - " + lines[1]);
                exportFile.WriteLine("charactersIndex.insert(pair<string,int>(\"" + lines[1] + "\"," + characterID + "));");
                for (int i = 0; i < 7; i++) exportFile.WriteLine("statBlocks[" + characterID + "]." + stats[i] + "=" + lines[i + 2] + ";");
                exportFile.WriteLine("");
                exportFile.WriteLine("//Stat Gains Block - Saved as the character's Stat gain block. Also used for level adjustments");
                for (int i = 0; i < 5; i++) exportFile.WriteLine("statGainBlocks[" + characterID + "]." + statGains[i] + "=" + lines[i + 13] + ";");
                exportFile.WriteLine("");
                exportFile.WriteLine("//Weapons to Equip, Armour to Equip, Abilities");
                string weaponToWrite = lines[9].Substring(4, lines[9].Length - 8);
                string armorToWrite = lines[11].Substring(4, lines[11].Length - 8);
                string abilityToWrite = lines[18].Substring(4, lines[18].Length - 8);
                string ability2ToWrite = lines[19].Substring(4, lines[19].Length - 8);
                string ability3ToWrite = lines[20].Substring(4, lines[20].Length - 8);
                exportFile.WriteLine("characterAttachments.push_back({\"" + weaponToWrite + "\",\"" + armorToWrite + "\",\"" + abilityToWrite + "\",\"" + ability2ToWrite + "\",\"" + ability3ToWrite + "\"});");
                exportFile.WriteLine("weaponDropChance.push_back(" + lines[10] + ");");
                exportFile.WriteLine("armorDropChance.push_back(" + lines[12] + ");");
                exportFile.WriteLine("");
                exportFile.WriteLine("//====================================================");
                exportFile.WriteLine("");
                characterID++;
            ...
            foreach (string weaponTXT in weaponList)
            {
                ...
                exportFile.WriteLine("weapons.push_back(WeaponComponent(" + weaponTypeExport + ",\"" + weaponNameExport + "\","+counter+"));");
            ...
            foreach (string armorTXT in armorList)
            {
                ...
                exportFile.WriteLine("//Item ID: " + counter + " - " + lines[1]);
                exportFile.WriteLine("armors.push_back(ArmorComponent(\"" + lines[1] + "\","+counter+"));");
                exportFile.Write("bodyParts.push_back({");
                string[] bodyParts =new string[]{"legs","rightArm","leftArm","Head"};
                for (int i = 21; i < 24; i++) exportFile.Write("pair<string,bool>(\"" + bodyParts[i - 21] + "\"," + lines[i] + "),");
                exportFile.Write("pair<string,bool>(\"" + bodyParts[3] + "\"," + lines[24] + ")});\n");
                exportFile.Write("resistances.push_back({");
                string[] elements = new string[] {"fire","ice","poison","dark","nature","wind","light","harmony"};
                for (int i = 6; i < 13; i++) exportFile.Write("pair<ElementType,int>(" + elements[i - 6] + "," + lines[i] + "),");
                exportFile.Write("pair<ElementType,int>(" + elements[7] + "," + lines[13] + ")});\n");
                exportFile.WriteLine("armors[" + counter + "].spawn(resistances[+"+counter+"], " +
                                                   lines[25] + ",\"" +
                                                   lines[26] +
                                                   "\",bodyParts["+counter+"]," +
